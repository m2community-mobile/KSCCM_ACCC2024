...
//
//  Created by JinGu-MacBookPro on 09/02/2019.
...
//    }()
    ...
    lazy var photoSize : CGSize = {
        ...
        let sizeWidth : CGFloat = (SCREEN.WIDTH - (edgeInset.left + edgeInset.right) - (2 * cellGap)) / 3
        let sizeHeight : CGFloat = sizeWidth
    ...
    override func prepare() {
        ...
        for i in 0..<numberOfitem {
            ...
            let originX : CGFloat = edgeInset.left + ((photoSize.width + cellGap) * (CGFloat(i % 3)))
            let originY : CGFloat = edgeInset.top + ((photoSize.height + cellGap) * (CGFloat(i / 3)))
            
            let kIndexPath = IndexPath(row: i, section: 0)
            let kAtt = UICollectionViewLayoutAttributes(forCellWith: kIndexPath)
            
            kAtt.frame = CGRect(origin: CGPoint(x: originX, y: originY), size: photoSize)
            
            layoutDic[kIndexPath] = kAtt
            
        }
        
        print("layoutDic:\(layoutDic)")
...
//        let height = edgeInset.top + (CGFloat(numberOfGroup) * (photoSize.height + cellGap)) + edgeInset.bottom
//        return CGSize(width: SCREEN.WIDTH, height: height )
        var heightComponnent = CGFloat(numberOfitem / 3)
        if numberOfitem % 3 != 0 {
            heightComponnent += 1
        }
        let height = edgeInset.top + (heightComponnent * (photoSize.height + cellGap)) + edgeInset.bottom
        return CGSize(width: SCREEN.WIDTH, height: height)
    }
