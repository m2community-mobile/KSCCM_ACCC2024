...
//                    })
    ...
    func userNotificationCenter(_ center: UNUserNotificationCenter, willPresent notification: UNNotification, withCompletionHandler completionHandler: @escaping (UNNotificationPresentationOptions) -> Swift.Void) {
        ...
        if !isLogin { completionHandler([]); return }
        ...
        if notification.request.trigger!.isKind(of: UNCalendarNotificationTrigger.self ){
            ...
            alertCon.addAction(UIAlertAction(title: "confrim", style: UIAlertAction.Style.default, handler: { (action) in
                appDel.allDismiss {
    ...
    func setToken(token : String){
        ...
        let _ = Server.postData(urlString: urlString, method: .post, otherInfo: sendDataDic, completion: { (kData:Data?) in
            if let data = kData {
                if let dataString = data.toString() {
                    print("success : \(dataString)")
//                    userD.set(dataString, forKey: USER_SID)
//                    userD.synchronize()
                }
            }
        })
...
//        ["time": "10:00-10:30", "subject": "JS%20MIN%20Memorial%20Lecture", "sid": "465", "tab": "39"]
        ...
        //월 세팅
//        if tab == "421" {
//            if let setTime = Calendar.current.date(bySetting: Calendar.Component.month, value: 3, of: startTime) {
//                startTime = setTime
//            }
//        }else{
