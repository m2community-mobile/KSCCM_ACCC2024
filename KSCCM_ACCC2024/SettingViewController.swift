...
let settingVCBackgroundColor = #colorLiteral(red: 1, green: 1, blue: 0.9999999404, alpha: 1)
let settingVCContentBackgroundColor = #colorLiteral(red: 0.9422092438, green: 0.9571908116, blue: 0.9742828012, alpha: 1)
...
class SettingViewController: BaseViewController {
    ...
    override func viewDidLoad() {
        ...
        let separatorView1 = UIView(frame: CGRect(x: 0, y: self.subTitleView.frame.maxY, width: SCREEN.WIDTH, height: 0))
        ...
        ////
        var viewTextTypeFontSize1 : CGFloat = 15
        var viewTextTypeFontSize2 : CGFloat = 12
//        let noticeView1Text = "Receive the latest information of the KSCCACCC 2023 in real time by clicking on the PUSH notification."
        let noticeView1Text = "Receive the latest information of the KSCCM-ACCC 2024 in real time by clicking on the PUSH notification."
...
//        let noticeView2Text = "Change your iPhone settings if you do not receive any notifications. iPhone > Setting > Notifications > Turn notifications for this app ‘ON’"
        ...
        let noticeView1 = ViewTextType(frame: CGRect(x: 0, y: separatorView1.frame.maxY, width: SCREEN.WIDTH, height: 300), text: noticeView1Text, fontSize: viewTextTypeFontSize1)
        noticeView1.layer.borderWidth = 0
        self.view.addSubview(noticeView1)
        
        let separatorView2 = UIView(frame: CGRect(x: 0, y: noticeView1.frame.maxY, width: SCREEN.WIDTH, height: 0))
        separatorView2.layer.borderWidth = 0
        ...
        let separatorView3 = UIView(frame: CGRect(x: 0, y: pushSettingView.frame.maxY, width: SCREEN.WIDTH, height: 0))
        separatorView3.backgroundColor = settingVCSeparatorColor
        self.view.addSubview(separatorView3)
        
        
        let noticeView2 = ViewTextType(frame: CGRect(x: 0, y: separatorView3.frame.maxY, width: SCREEN.WIDTH, height: 500), text: noticeView2Text, fontSize: viewTextTypeFontSize2)
        noticeView2.layer.borderWidth = 0
        self.view.addSubview(noticeView2)
        
        let separatorView4 = UIView(frame: CGRect(x: 0, y: noticeView2.frame.maxY, width: SCREEN.WIDTH, height: 0))
        ...
        appVersionView.layer.borderWidth = 0
//        appVersionView.frame.origin.y = separatorView4.frame.maxY
        appVersionView.frame.origin.y = self.view.frame.size.height / 1.2
        self.view.addSubview(appVersionView)
        
        let separatorView5 = UIView(frame: CGRect(x: 0, y: appVersionView.frame.maxY, width: SCREEN.WIDTH, height: 0))
...
//        logoutButton.backgroundColor = mainColor
        logoutButton.backgroundColor = #colorLiteral(red: 0.239446789, green: 0.264031291, blue: 0.2948754728, alpha: 1)
...
class ViewSwitchType: UIView {
    ...
    init(itemName : String, tintColor : UIColor) {
        super.init(frame: CGRect(x: 20, y: 0, width: SCREEN.WIDTH - 40, height: 50))
        ...
        onOffButton = UISwitch(frame: CGRect(x: SCREEN.WIDTH - 40 - 30 - 40, y: 0, width: 40, height: 30))
        onOffButton.center.y = self.frame.size.height / 2
        onOffButton.onTintColor = tintColor
        onOffButton.onTintColor = #colorLiteral(red: 0.9522905946, green: 0.403362751, blue: 0, alpha: 1)
...
class ViewAppVersionType: UIView {
    ...
    init(itemName : String, tintColor : UIColor) {
        super.init(frame: CGRect(x: 20, y: 20, width: SCREEN.WIDTH - 40, height: 50))
        self.backgroundColor = #colorLiteral(red: 0.9529412389, green: 0.9529412389, blue: 0.9529411197, alpha: 1)
        
        self.layer.borderWidth = 0
        
        let itemLabel = UILabel(frame: CGRect(x: 40, y: 0, width: SCREEN.WIDTH - 20, height: self.frame.size.height))
        itemLabel.text = itemName
        itemLabel.font = UIFont(name: Nanum_Barun_Gothic_OTF_Bold, size: 15)
        itemLabel.textColor = #colorLiteral(red: 0, green: 0, blue: 0, alpha: 1)
        self.addSubview(itemLabel)
        ...
        currentVersionLabel.center.y = self.frame.size.height / 2
        currentVersionLabel.frame.origin.x = SCREEN.WIDTH - currentVersionLabel.frame.size.width - 55
        
        let updateButton = UIButton(frame: CGRect(x: 0, y: 0, width: 70, height: 30))
        updateButton.frame.origin.x = currentVersionLabel.frame.minX - updateButton.frame.size.width - 25
        ...
        updateButton.titleLabel?.font = UIFont.systemFont(ofSize: 11)
