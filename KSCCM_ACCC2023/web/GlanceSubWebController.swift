...
,WKNavigationDelegate , WKUIDelegate
{
    ...
    override func viewDidLoad() {
        ...
        self.wkWebView = WKWebView(frame: CGRect(x: 0, y: STATUS_BAR_HEIGHT, width: SCREEN.WIDTH, height: SCREEN.HEIGHT - STATUS_BAR_HEIGHT - SAFE_AREA))
        ...
        if gesture.state == .began {
            ...
            self.wkWebView.readImageFrom(point: CGPoint(x: self.lastX, y: self.lastY)) { (image) in
                if let image = image {
                    ...
                    actionSheet.addAction(UIAlertAction(title: "취소", style: UIAlertAction.Style.cancel, handler: { (action) in
                        
                    }))
                    actionSheet.addAction(UIAlertAction(title: "이미지 저장하기", style: UIAlertAction.Style.default, handler: { (action) in
